...
// SVG 받으면 emoji 대신 <img src={...} /> 로 교체하면 됩니다
// id는 store에 저장되는 키값이므로 변경 금지
const MOODS = [
...
export default function Diary() {
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", gap: 12, padding: "22px 16px 16px", overflowY: "auto" }}>
      {/* 오늘 입력 */}
      <div style={card}>
        {/* 기분 */}
        <p style={{ fontSize: 13, fontWeight: "normal", color: "var(--text)", margin: "8px 0 16px", textAlign: "center" }}>
          오늘 기분은?
        </p>
        <div style={{ display: "flex", gap: 8, marginBottom: 14, justifyContent: "center" }}>
          {MOODS.map(m => (
            <button key={m.id} onClick={() => { setMood(m.id); setSaved(false); }} style={{
              display: "flex", flexDirection: "column", alignItems: "center", gap: 2,
              padding: "6px 8px", borderRadius: 10, cursor: "pointer",
              border: mood === m.id ? "1px solid var(--pink)" : "1px solid transparent",
              background: mood === m.id ? "rgba(244,192,209,0.15)" : "transparent",
              transform: mood === m.id ? "scale(1.1)" : "scale(1)",
              transition: "all 0.15s",
            }}>
              {/* SVG 받으면 아래 span을 <img src={`/moods/${m.id}.svg`} width={24} height={24} /> 로 교체 */}
      ...
      <div style={card}>
        ...
        {[...diary].reverse().slice(0, 10).map(entry => {
          ...
          <div key={entry.date} style={{
            ...
            {/* SVG 받으면 아래를 <img> 로 교체 */}
            <span style={{ fontSize: 16, flexShrink: 0 }}>{moodObj?.emoji || "📝"}</span>
            <div>
